...
function init() {
  /* utiliser les événements tactiles si elles sont prises en charge, autrement utiliser les événements de souris*/
...
/* Créer le tableau et le joueur initial */
  createBoard();
  setInitialPlayer();
}
...
function squareSelected(evt, currentPlayer) {
  ...
  if (square.className.match(/marker/)) {
    alert("Desole, cet espace est pris. Selectionner un carre libre.");
    return;
  }
...
function declareWinner() {
  if (confirm("Yeah! GAGNE!!!  Nouvelle partie?")) {
    newGame();
  }
}
...
function checkForWinner() {
  ...
  if (!JSON.stringify(board).match(/,"",/)) {
    if (confirm("c'est perdus. Nouvelle partie?")) {
      newGame();
    }
  }
}
